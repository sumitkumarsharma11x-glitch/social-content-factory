...
import { registerApproveAllRoute } from "./approve-all-route";

const PORT=Number(process.env.PORT)||3000;
const DB_PATH=process.env.DB_PATH||"./social-content-factory.sqlite3";
export function createApp(){
  const app=express(); app.use(express.json());
  const publicDir=path.resolve(__dirname,"../public"); app.use(express.static(publicDir));
  const db=openDatabase(DB_PATH);
  registerGenerateBatchRoute(app,db); registerReviewDashboardRoutes(app,db); registerVideoFactoryRoutes(app,db);
  registerVideoRenderRoutes(app,db,path.join(publicDir,"videos")); registerApproveAllRoute(app,db);
  app.get("/health",(_req,res)=>res.json({ok:true})); app.get("/",(_req,res)=>res.sendFile(path.join(publicDir,"index.html")));
  return app;
}
