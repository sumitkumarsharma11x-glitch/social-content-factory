...
function toInput(piece: PieceRecord) { return { id: piece.id, platform: piece.platform, piece_number: piece.piece_number, title: piece.title, body: piece.body, hashtags: piece.hashtags, cta: piece.cta }; }
export async function handlePieceVideoRenderRequest(pieceId: number, publicVideoDir: string, deps: { db: Database.Database }, targetDurationSeconds?: number): Promise<{ status: number; result: VideoRenderSuccess | VideoRenderFailure }> {
  ...
  try {
    ...
    if (Number.isFinite(targetDurationSeconds) && (targetDurationSeconds ?? 0) >= 30) input.target_duration_seconds = Math.min(900, Math.round(targetDurationSeconds!));
    return { status: 200, result: { ok: true, video: await renderVideoForPiece(input, publicVideoDir) } };
  } catch (error) { return { status: 500, result: { ok: false, message: error instanceof Error ? error.message : "Video rendering failed." } }; }
}
export async function handleBatchVideoRenderRequest(batchId: number, publicVideoDir: string, deps: { db: Database.Database }, targetDurationSeconds?: number): Promise<{ status: number; result: VideoBatchRenderSuccess | VideoRenderFailure }> {
  ...
  } catch (error) { return { status: 500, result: { ok: false, message: error instanceof Error ? error.message : "Batch video rendering failed." } }; }
}
export function registerVideoRenderRoutes(router: Router, db: Database.Database, publicVideoDir: string): void {
  router.post("/api/pieces/:pieceId/video", async (req: Request, res: Response) => {
    ...
    if (!Number.isInteger(pieceId) || pieceId < 1) { res.status(400).json({ ok: false, message: "pieceId must be a positive integer." }); return; }
